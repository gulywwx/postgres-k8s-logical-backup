...
if [ "$LOGICAL_BACKUP_PROVIDER" == "az" ]; then
    ...
    exit $ERRORCOUNT
elif [ "$LOGICAL_BACKUP_PROVIDER" == "fs" ]; then
    PATH_TO_BACKUP=$LOGICAL_BACKUP_FS_PATH"/logical_backups/"$(date +%Y-%m-%dT%H%M%S)
    test ! -d $PATH_TO_BACKUP && mkdir -p $PATH_TO_BACKUP

    echo Dumping and uploading global items...
    dump_global | compress > $PATH_TO_BACKUP/global.sql.gz
    ...
    while read dbname; do 
        ...
        dump_db $dbname
        echo Uploading directory /tmp/db-$dbname to $PATH_TO_BACKUP/$dbname...
        cp -rp /tmp/db-$dbname $PATH_TO_BACKUP/$dbname
        echo Cleaning up /tmp/db-$dbname...
        rm -rf /tmp/db-$dbname
    done < /tmp/database-list
